                    ...
                    HStack {
                        ForEach(element.bars) { bar in
                            GeometryReader { geometry in
                                VStack {
                                        Rectangle()
                                            .frame(width: 14, height: self.height(for: bar, viewHeight: geometry.size.height, maxValue: self.maxDataSetValue))
                                            .cornerRadius(6, antialiased: false)
...
struct ParentView: View {
    ...
    //            .previewLayout(.sizeThatFits)
            ...
            Text("Selected element \(selectedElement?.bars[0].value ?? 0)")
        }
        .padding()
    }
}
